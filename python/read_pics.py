...
if __name__ == "__main__":
	...
	'''
	#Boucle while pour choisir la lettre à traiter
	while(1):

	    fichier = ''
	    lettre = ''
	    lettre = input('fichier : ').upper()
	    
	    fichier = "pics_" + lettre + ".bin"#Saisi du fichier desiré
	    graph, info = get_pics_from_file("../data/"+fichier) #Recuperation des donné du .bin 
	    #print(graph)
	    ...
	    plt.title(fichier + ' densité')
	    plt.ylim(0, 1.5)
	    plt.grid(b=True, which='both')
	'''

	#Traitement densité sur chaque .bin
	fichiers_liste = [f for f in listdir("../data/") if isfile(join("../data/", f))]
	for i in range(len(fichiers_liste)):
	    ...
	    plt.ylim(0, 1.5)
	    plt.grid(b=True, which='both')
	    plt.savefig(nom) #SAVE à la racine du programme 
